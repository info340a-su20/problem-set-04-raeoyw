...
*/
function compoundInterest(bankBalance, interestRate, numYear) {
  return bankBalance * (Math.exp(interestRate * numYear));
}
...
function containsQueenOfHearts(cards) {
  for(let i=0; i<cards.length; i++) {
    if(cards[i].rank == 11 || cards[i].rank == 12 || cards[i].rank == 13 || cards[i].rank == 14) {
      return true;
    } 
  }
  return false;
